...
health_response=$(curl -s -w "\\nStatus_Code:%{http_code}" "${GOSWITCH_SERVER_URL}/health")
...
signin_response=$(curl -s -X POST -H "Content-Type: application/json" -H "api-key: hyperswitch" -H "User-Agent: HyperSwitch-Shell-Client/1.0" -H "Referer: ${GOSWITCH_CONTROL_CENTER_URL}/" -d "${signin_payload}" "${GOSWITCH_SERVER_URL}/user/signin")
...
if [[ $(
    ...
    signup_cmd="curl -s -X POST '${GOSWITCH_SERVER_URL}/user/signup' \
        -H 'Accept: */*' \
        -H 'Accept-Language: en-GB,en-US;q=0.9,en;q=0.8' \
        -H 'Content-Type: application/json' \
        -H 'Origin: ${GOSWITCH_CONTROL_CENTER_URL}' \
        -H 'Referer: ${GOSWITCH_CONTROL_CENTER_URL}/' \
...
if [ "${token_type}" = "totp" ]; then
    ...
    for i in $(seq 1 ${MAX_RETRIES}); do
        terminate_response=$(curl -s -X GET -H "Content-Type: application/json" -H "api-key: hyperswitch" -H "authorization: Bearer ${token}" "${GOSWITCH_SERVER_URL}/user/2fa/terminate?skip_two_factor_auth=true")
...
# Get user info
if [ -n "${token}" ]; then
    user_info_cmd="curl -s -X GET -H 'Content-Type: application/json' -H 'api-key: hyperswitch' -H 'authorization: Bearer ${token}' '${GOSWITCH_SERVER_URL}/user'"
...
if [ "${is_new_user}" = true ] && [ -n "${merchant_id}" ] && [ -n "${token}" ]; then
    ...
    merchant_response=$(curl -s -X POST -H "Content-Type: application/json" -H "api-key: hyperswitch" -H "authorization: Bearer ${token}" -d "${merchant_payload}" "${GOSWITCH_SERVER_URL}/accounts/${merchant_id}")
...
EOF
    ...
    connector_response=$(curl -s -X POST -H "Content-Type: application/json" -H "api-key: hyperswitch" -H "authorization: Bearer ${token}" -d "${connector_payload}" "${GOSWITCH_SERVER_URL}/account/${merchant_id}/connectors")
